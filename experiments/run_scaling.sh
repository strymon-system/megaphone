...
for i in `seq 1 10`; do
  ...
  for p in `seq 0 1`; do
    ssh -t andreal@sgs-r820-01 "cd /home/andreal/Src/dynamic-scaling-mechanism; hwloc-bind socket:$p -- cargo run --release --example word_count -- $rounds $batch $keys open-loop -n 2 -p $p -w 1" > results/word_count_${COMMIT}/$filename &
  done
  wait
done
