...
def sigmod_micro_migr_throughput(group, groups=1):
    workers = 4
    processes = 4
    duration = 1200
    bin_shift = 12
    # Throughout feasibility experiment
    # Domain is 4M per bin, 2**12 bins
    domain = 4000000 * 4096
    # Vary rate from 250k to 32M
    for rate in [int(x) * 1000000 for x in [.25, .5, 1, 2, 4, 8, 16, 32]]:
        for migration in ["sudden", "fluid", "batched"]:
            experiment = Experiment(
